...
save_to_camera_callbeck = None

def save_to_camera():
    global save_thr
    global thresholds  
    global save_to_camera_callbeck 
    print("dfgdfghfh")
    save_thr = False

    if hasattr(thresholds, 'tolist'):  # проверка, что это numpy массив
        thresholds_to_save = thresholds.tolist()
    else:
        thresholds_to_save = thresholds
    with open("thresholds.txt", "w") as file:
        file.write(str(thresholds_to_save))

    if save_to_camera_callbeck is not None and callable(save_to_camera_callbeck):
        save_to_camera_callbeck(thresholds_to_save)   # можно передать сохранённое значение порога
